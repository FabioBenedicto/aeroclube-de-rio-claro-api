...
export class PayableTypesService {
  constructor(private readonly repo: PayableTypesRepository) {}
  ...
  async findById(id: number) {
    ...
    if (!type) throw new NotFoundException(`Tipo de pagável ${id} não encontrado`);
    return type;
  }

  async findByName(name: string) {
    const type = await this.repo.findByName(name);
    if (!type) throw new NotFoundException(`Tipo de pagável '${name}' não encontrado`);
    return type;
  }
