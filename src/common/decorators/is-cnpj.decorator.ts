...
function isCNPJ(value: string): boolean {
  ...
  if (hasAllCharsEqual) return false;

  const firstDigitSum = FIRST_DIGIT_WEIGHTS.reduce(
    (acc, m, i) => acc + charsCode[i] * m,
    0,
  );

  const firstDigit = firstDigitSum % 11 < 2 ? 0 : 11 - (firstDigitSum % 11);

  const secondDigitSum = SECOND_DIGIT_WEIGHTS.reduce(
    (acc, m, i) => acc + charsCode[i] * m,
    0,
  );
